"""CachedTravelAgent - one Strands agent with the three cache hooks.

Same design as the notebook: a single ``Agent`` with ResponseCache,
ToolResultCache, and ReasoningCache attached. No wrapper logic around the agent;
the caching happens inside the loop through Strands hook events.

    agent = CachedTravelAgent(CacheConfig())
...
class CachedTravelAgent:
    ...
    caches attached as hooks on one agent, on one DynamoDB table.

    Args:
        cfg: The CacheConfig describing table, models, and thresholds.
        cache_mode: "both" (default, all three), "response-cache" (response only),
            or "reasoning-cache" (tool-result + reasoning only).
    """

    def __init__(self, cfg: CacheConfig, cache_mode: str = "both"):
        self._cfg = cfg
        self._cache_mode = cache_mode
        self._ddb = boto3.client("dynamodb", region_name=cfg.region)
        self._model = None
        self._response = ResponseCache(self._ddb, cfg)
        self._tools = ToolResultCache(self._ddb, cfg)
        self._reasoning = ReasoningCache(self._ddb, cfg)
