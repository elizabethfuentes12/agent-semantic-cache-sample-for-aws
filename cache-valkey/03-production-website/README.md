# 03 · Production Website: Real-Time Web Chat for the Valkey-Backed Agent
...
and `frontend/` (a static dashboard page on S3 + CloudFront). Based on the AppSync
...
Reads: `/semantic-cache/agent-runtime-arn` (plus the plan-cache runtime ARN)
- `generate_config.sh` reads these from SSM and writes them into the
dashboard's `config.js` at deploy time.
