# 03 Backend · AppSync Events + Lambda Router (Valkey track)

> The website backend for the Valkey-track production agent. The same
> AppSync Event–Lambda primitive is reused by the [DynamoDB track](../../../cache-dynamodb/03-production-website/); only the agent ARNs it targets differ.
...
graph TD
    ...
    subgraph APPSYNC["AppSync Events API"]
        ...
        CACHE_NS["cache/ namespace"]
    end

    subgraph LAMBDAS["Lambda Functions"]
        PUB_LAMBDA[Publish Lambda]
        SUB_LAMBDA[Subscribe Lambda]
        CHAT_LAMBDA[Chat History Lambda]
        CACHE_LAMBDA[Cache Inventory Lambda]
    end
    ...
    CACHE_LAMBDA -->|proxy invoke| TARGET_LAMBDA
    CACHE_LAMBDA -->|read config| SSM_STORE
...
├── lambdas/
│   ├── project_lambdas.py                     # Lambdas construct (publish + subscribe + chat_history + cache_inventory)
│   └── code/
...
│       ├── subscribe/                         # Subscribe authorizer Lambda (response channel)
│       ├── chat_history/                      # Chat History Lambda
│       │   ├── lambda_function.py             # CRUD action router
│       │   ├── chat_history_service.py        # DynamoDB CRUD operations
│       │   └── config_service.py              # SSM parameter retrieval helper
│       └── cache_inventory/                   # Cache inventory Lambda (proxies to stack-01 reasoning fn)
...
- Cognito User Pool: by default the stack **creates its own** (`COGNITO_USER_POOL_ID = None` in `config.py`). Set it to an existing pool ID to reuse one instead.
...
| `response/` | Subscribe Lambda | REQUEST_RESPONSE | Receive streaming responses (subscribe authorization) |
| `chat/` | Chat History Lambda | REQUEST_RESPONSE | CRUD operations on chat history |
| `cache/` | Cache Inventory Lambda | REQUEST_RESPONSE | Live cache inventory (proxies to the stack-01 reasoning Lambda) |
...
```
/{RESPONSE_NAMESPACE}{original_channel_path}/{event_id}
```

Example: publishing to `messages/default` produces responses on `/response/messages/default/{event_id}`. AppSync Events channel paths carry a leading slash, so the published channel is prefixed with `/` (the incoming path is normalized with `lstrip("/")` first to avoid a doubled slash).
...
The AppSync Events API always enables API key auth. Cognito User Pool auth is
also enabled: by default the stack **creates its own** User Pool
(`COGNITO_USER_POOL_ID = None`); set it to an existing pool ID to reuse one.
...
| Parameter | Value |
|-----------|-------|
| `/semantic-cache/appsync/http_endpoint` | AppSync HTTP DNS |
| `/semantic-cache/appsync/realtime_endpoint` | AppSync Realtime DNS |
| `/semantic-cache/appsync/api_key` | AppSync API key |
| `/semantic-cache/bucket/media_sessions` | S3 bucket name |
| `/semantic-cache/table/chat_messages` | DynamoDB table name |
...
| `/auth/user_pool_id` | Cognito User Pool ID (when the stack owns the pool) |
...
Edit `config.py` to control the Cognito User Pool:

```python
# None (default): the stack creates and owns its own User Pool.
# Set to an existing Cognito User Pool ID to reuse it instead.
...
| Test File | Covers |
|-----------|--------|
| `test_publish_lambda.py` | Routing, validation, persistence integration, response channel format |
...
| `test_config_service.py` | SSM parameter retrieval helper |
| `test_appsync_service.py` | AppSync HTTP publish client |
| `test_agent_core_service.py` | AgentCore runtime client + SSE reader |
| `test_events_api_construct.py` | EventsAPI construct (auth providers, namespaces) |
| `test_stack.py` / `test_appsync_event_lambda_primitive_stack.py` | CDK template assertions |

> Run pytest from this `backend/` directory: the CDK assertion tests bundle
> Lambda assets by relative path.
