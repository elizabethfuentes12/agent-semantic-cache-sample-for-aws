...
class TestChatHistoryLambda:
    ...
    def test_two_lambda_functions_exist(self):
        """publish, subscribe, chat_history, and cache_inventory Lambdas should be created."""
        _, template = _make_stack()
...
class TestChatChannelNamespace:
    ...
    def test_three_channel_namespaces_exist(self):
        """Four channel namespaces should exist: messages, response, chat, cache."""
        _, template = _make_stack()
        template.resource_count_is("AWS::AppSync::ChannelNamespace", 4)
                ...
                "Statement": assertions.Match.array_with([
                    assertions.Match.object_like({
                        ...
                        "Resource": assertions.Match.string_like_regexp(
                            ".*parameter/semantic-cache/table/chat_messages"
                        ),
                    }),
                ]),
            }),
