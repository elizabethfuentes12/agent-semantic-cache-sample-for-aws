...
def _reset_mock():
    """Force config_service to use the mock SSM client for every test.

    Patching the module attribute directly (rather than relying on
    sys.modules.setdefault at import time) makes these tests deterministic
    regardless of whether another test already imported the real boto3.
    """
    _mock_ssm_client.reset_mock()
    _mock_ssm_client.get_parameter.side_effect = None
    with patch.object(config_service, "ssm_client", _mock_ssm_client):
        yield
