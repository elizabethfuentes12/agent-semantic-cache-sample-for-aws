# 02 · Production Agent on AgentCore Runtime with Valkey Cache: VPC-Attached

The stack-01 travel agent, promoted to production: two Strands Agents runtimes
(the production agent plus the plan-cache agent) running
...
| `agent_files/plan_cache_agent.py` | The plan-template cache agent, its own AgentCore runtime |
...
Writes (read by stack 03): `/semantic-cache/agent-runtime-arn`, and the
advanced-agent ARN `/semantic-cache/plan-cache-runtime-arn`
