...
Wikipedia REST): no hardcoded data. `search_flights` also calls the Duffel
sandbox, whose API key is read from AWS Secrets Manager (never an env var);
the other three tools need no key.
...
Example warm-run output (shape of the response; token split is illustrative,
the measured total is below):

```json
{
  "answer": "...",
  "cycles": 2,
  "usage": {"totalTokens": 2965},
  "plan_hint_used": true,
  "tool_cache_hits": 3,
  "tool_executions": 0,
  "latency_ms": 6749
...
|----------|--------|---------|
| VALKEY_HOST / VALKEY_PORT | stack wiring | Node-based ElastiCache endpoint (vector search: trajectories) |
| TOOL_CACHE_HOST / TOOL_CACHE_PORT | stack wiring | ElastiCache Serverless endpoint (exact-match tool-result cache) |
...
`bedrock:InvokeModel(WithResponseStream)` and `secretsmanager:GetSecretValue`
(the Duffel key for `search_flights`). Outbound internet via NAT for the
