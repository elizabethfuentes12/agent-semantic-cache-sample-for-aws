...
class SemanticCacheStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # Cross-stack contract: 02 (production agent) and 03 (website) read
        # everything they need from these parameters — no direct references.
        private_subnets = net.vpc.select_subnets(
            subnet_type=ec2.SubnetType.PRIVATE_WITH_EGRESS
        ).subnet_ids
        SharedParams(self, "Params", values={
            "valkey-host": cache.endpoint_address,
            "valkey-port": cache.endpoint_port,
            "tool-cache-host": tool_cache.endpoint_address,
            "tool-cache-port": tool_cache.endpoint_port,
            "vpc-id": net.vpc.vpc_id,
            "private-subnet-ids": ",".join(private_subnets),
            "agent-runtime-sg-id": net.agent_runtime_sg.security_group_id,
