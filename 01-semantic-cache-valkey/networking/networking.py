...
class Networking(Construct):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # Stack 02 attaches this SG to the AgentCore Runtime ENIs so the
        # production agent reaches both cache stores directly (option A).
        self.agent_runtime_sg = ec2.SecurityGroup(
            self,
            "AgentRuntimeSg",
            vpc=self.vpc,
            description="AgentCore Runtime ENIs (production agent)",
            allow_all_outbound=True,
        )
        self.cache_sg.add_ingress_rule(
            peer=self.agent_runtime_sg,
            connection=ec2.Port.tcp(6379),
            description="Valkey from the AgentCore Runtime",
        )
